...
// A let-scoped variable isn't shared across module reloads.
// Use a file-based PID lock instead of globalThis for durability.
...
const lockPath = join(homedir(), ".config", "opencode", "telegram-remote", "bot.lock");
...
function releaseLock() {
  try { writeFileSync(lockPath, "", { mode: 0o600 }); } catch {}
}
  ...
  const onMessage = async (incomingChatId: number, text: string) => {
    ...
    if (text.startsWith("/")) {
      ...
      switch (cmd) {
        case "/start":
        case "/help": {
          const helpText = `<b>🤖 JA OpenCode Telegram</b>
...
/health — System status with model + stats`;
        ...
        case "/models": {
          ...
          parts.push(``, `<b>Available models:</b>`, ...maxDisplay.map(m =>
            `${m === currentModel ? "✅" : "  "} <code>${escapeHtml(m)}</code>`
          ));
    ...
    try {
      // Use OpenCode SDK client to send message in-process
      // Each chat gets its own session to isolate context
      const sessionId = `tg:${incomingChatId}`;
      
      // First, try creating a fresh session (no parent context)
      let response = "";
      try {
        // Create a fresh session for this message context
        const createResult = await input.client.session.create({
          body: {
            title: `Telegram from ${incomingChatId}`,
            parentID: undefined, // Fresh, no inherited context
          },
          query: {
            directory: input.directory || process.cwd(),
          },
        });
        const freshSessionId = (createResult as any)?.id || sessionId;
        
        // Send the prompt to the fresh session
        const result = await input.client.session.prompt({
          path: { id: freshSessionId },
          body: {
