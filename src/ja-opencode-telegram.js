...
function loadEnv() {
    const envPaths = [
        (0, path_1.join)((0, os_1.homedir)(), ".config", "opencode", "telegram-remote", ".env"),
        (0, path_1.join)((0, os_1.homedir)(), ".config", "opencode", ".env"),
    ];
    for (const p of envPaths) {
        if ((0, fs_1.existsSync)(p)) {
            const content = (0, fs_1.readFileSync)(p, "utf-8");
...
// ─── State Store ─────────────────────────────────────────────────────────────
const statePath = (0, path_1.join)((0, os_1.homedir)(), ".config", "opencode", "telegram-remote", "state.json");
function readState() {
    try {
...
function writeState(state) {
    (0, fs_1.mkdirSync)((0, path_1.dirname)(statePath), { recursive: true, mode: 0o700 });
    // Migrate: remove legacy fields
    const clean = { lastUpdateId: state.lastUpdateId ?? 0, chatId: state.chatId ?? null };
    (0, fs_1.writeFileSync)(statePath, JSON.stringify(clean, null, 2), { mode: 0o600 });
}
// ─── Logger ──────────────────────────────────────────────────────────────────
// ─── PID Lock ────────────────────────────────────────────────────────────────
// File-based lock to prevent duplicate bot instances across hot-reloads
const lockPath = (0, path_1.join)((0, os_1.homedir)(), ".config", "opencode", "telegram-remote", "bot.lock");
function acquireLock() {
    try {
        const existing = (0, fs_1.readFileSync)(lockPath, "utf-8").trim();
        if (existing) {
            try {
                process.kill(parseInt(existing, 10), 0); // check if alive
    ...
    (0, fs_1.writeFileSync)(lockPath, String(process.pid), { mode: 0o600 });
    return true;
}
function releaseLock() {
    try {
        (0, fs_1.writeFileSync)(lockPath, "", { mode: 0o600 });
    }
    catch { }
}
const logPath = (0, path_1.join)((0, os_1.tmpdir)(), "opencoder-telegram.log");
...
// ─── OpenCode Config ─────────────────────────────────────────────────────────
const configPath = (0, path_1.join)((0, os_1.homedir)(), ".config", "opencode", "opencode.json");
...
class TelegramBot {
    ...
    consecutive409s = 0;
    constructor(token, allowedUserIds, initialChatId, onMessage) {
        ...
        if (initialChatId)
            this.state.chatId = initialChatId;
        this.onMessage = onMessage;
    }
    polling = false;
    async killOldConnection() {
        try {
            // Delete webhook to clear stale polling connections
            // Do NOT drop_pending_updates — that would destroy unprocessed messages
            await fetch(`${this.base}/deleteWebhook`);
            await new Promise(r => setTimeout(r, 500));
            // Use offset=-1 to discover latest pending update_id
            ...
            if (res.ok) {
                ...
                if (data.ok && data.result?.length > 0) {
                    ...
                    // Set offset one below so the next poll with (offset+1) confirms it
                    this.offset = latestId - 1;
                    this.state.lastUpdateId = latestId - 1;
                    writeState(this.state);
                    log("info", "Discovered pending update, adjusting offset", { latestId, newOffset: this.offset });
                    return;
                }
            }
            // No pending updates — use saved offset (or 0)
            this.offset = this.state.lastUpdateId || 0;
            log("info", "No pending updates, using saved offset", { offset: this.offset });
        }
        catch (err) {
    ...
    async sendMessage(chatId, text, options) {
        try {
            ...
            if (!res.ok) {
                ...
                await res.json(); // consume body for GC
            }
        }
        catch (err) {
    ...
    async handleCallback(chatId, callbackId, data, messageId) {
        ...
        if (type === "p" && parts.length >= 3) {
            ...
            const resolvedAction = action === "r" ? "deny" : "allow";
            const resolvedMode = action === "a" ? "always" : "once";
    ...
    async askPermission(chatId, permission) {
...
const plugin = async (input) => {
    // Guard: PID file lock prevents multiple active bot instances
    if (!acquireLock()) {
        log("info", "Another bot instance is running — skipping duplicate instantiation");
        return {};
    }
    ...
    const onMessage = async (incomingChatId, text) => {
        try {
...
/health — System status with model + stats`;
                    ...
                    case "/models": {
                        ...
                        const aliasLines = Object.entries(MODEL_ALIASES).map(([alias, id]) => `${id === currentModel ? "✅" : "  "} <code>${alias}</code> → <code>${escapeHtml(id)}</code>`);
                        const parts = [
                            `<b>📦 Models</b>`,
                            ``,
                            `Current: <code>${escapeHtml(currentModel || "(not set)")}</code>`,
                        ];
                        // Only show first 20 models to avoid hitting Telegram's 4096 char limit
                        const maxDisplay = allModels.slice(0, 20);
                        if (allModels.length > 20) {
                            maxDisplay.push(`<i>… and ${allModels.length - 20} more</i>`);
                        }
                        parts.push(``, `<b>Available models:</b>`, ...maxDisplay.map(m => `${m === currentModel ? "✅" : "  "} <code>${escapeHtml(m)}</code>`));
                        parts.push(``, `<b>Shortcuts:</b>`, ...aliasLines);
            ...
            try {
                switch (type) {
                    ...
                    case "session.idle": {
                        ...
                        if (evt.id && evt.id !== evt.rootId)
                            return;
                        if (status === "idle") {
                            await bot.sendMessage(currentChatId, `✅ <b>Done:</b> ${escapeHtml(sessionTitle)}${agent ? ` (${escapeHtml(agent)})` : ""}`);
                        }
                        break;
                    }
                    case "question.asked": {
