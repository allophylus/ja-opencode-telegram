    ...
    while (!shuttingDown) {
        ...
        try {
            ...
            if (updates.length > 0) {
                ...
                // Pre-process: auto-abort session before model changes
                let hasModelAction = false;
                for (const u of updates) {
                    if (u.callback_query?.data?.startsWith('model:')) {
                        hasModelAction = true;
                        break;
                    }
                }
                if (hasModelAction) {
                    ...
                    http.get('http://127.0.0.1:4096/session', { timeout: 2000 }, (res) => {
                        let data = '';
                        res.on('data', c => data += c);
                        res.on('end', () => {
                            try {
                                ...
                                for (const s of ss) {
                                    const r = http.request('POST', 'http://127.0.0.1:4096/session/' + encodeURIComponent(s.id) + '/abort', { timeout: 2000 });
                                    r.end();
                                }
                            } catch (e) {}
                        });
                ...
                console.log('[UPDATE] Processing ' + updates.length + ' update(s)' + (hasModelAction ? ' (auto-aborted)' : ''));
                try {
